...
public class ProdutoController {
    ...
    public void setar_campos() {  //Método setar campos para os txt
        int setar = view.getTblProdutos().getSelectedRow();
        view.getTxtId().setText(view.getTblProdutos().getModel().getValueAt(setar, 0).toString());
        view.getTxtDescricao().setText(view.getTblProdutos().getModel().getValueAt(setar, 1).toString());
        view.getTxtValor().setText(view.getTblProdutos().getModel().getValueAt(setar, 2).toString());
        view.getJcbFid().removeAllItems();
        view.getJcbFid().addItem(view.getTblProdutos().getModel().getValueAt(setar, 3).toString());
        view.getJcbStatus().removeAllItems();
         if (view.getTblProdutos().getModel().getValueAt(setar, 4).equals(1)) {
            view.getJcbStatus().removeAllItems();
            view.getJcbStatus().addItem("ATIVO");
        } else if (view.getTblProdutos().getModel().getValueAt(setar, 4).equals(2)) {
            view.getJcbStatus().removeAllItems();
            view.getJcbStatus().addItem("INATIVO");
        }
        view.getJcbCategoria().removeAllItems();
        view.getJcbCategoria().addItem(view.getTblProdutos().getModel().getValueAt(setar, 5).toString());           
    }
    ...
    public void atualizarGrid(){
        ProdutoDao dao = new ProdutoDao();
        view.getTblProdutos().setModel(DbUtils.resultSetToTableModel(dao.consultar_produto()));
        //omitir campo de coluna
        view.getTblProdutos().getColumn(view.getTblProdutos().getColumnName(0)).setMinWidth(0);
        view.getTblProdutos().getColumn(view.getTblProdutos().getColumnName(0)).setMaxWidth(0);
        //tblProdutos.getColumn(tblProdutos.getColumnName(0)).setPreferredWidth(0);
        view.getTblProdutos().getColumn(view.getTblProdutos().getColumnName(1)).setPreferredWidth(150);
        view.getTblProdutos().enable(true);
    }    
}
